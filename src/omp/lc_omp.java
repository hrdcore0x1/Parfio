...
public final class lc_omp {
	...
	public static class Work {
		...
		public Work(IWork w, int start, int end, int step, int task, int tid,
				int n, CountDownLatch c) {
			where = w;
			this.start = start;
			this.end = end;
			this.step = step;
	...
	// all N threads execute the same task and return only after all tasks
	// complete
	...
	public static final void work(IWork w, int start, int end, int step,
			int taskid) {
		work(w, start, end, step, taskid, N, STATIC, 0);
	}

	public static final void work(IWork w, int start, int end, int step,
			int taskid, int nThreads) {
		work(w, start, end, step, taskid, nThreads, STATIC, 0);
	}

	public static final void work(IWork w, int start, int end, int step,
			int taskid, int nThreads, int schedule, int chunk) {
		...
		try {
			...
			case STATIC:
				if (chunk == 0) {
					...
					for (int i = 1; i < nThreads; i++) {
						work = new Work(w, start, start + k - 1, step, taskid,
								i - 1, nThreads, c); // TODO should be clone
														// method in IWork so
														// user can sub-class
														// Work
						q.put(work);
					...
					work = new Work(w, start, end, step, taskid, nThreads - 1,
							nThreads, c);
					q.put(work);
				} else
					assert (false);
			...
			case DYNAMIC: {
				...
				int i = 0;
				while (--k >= 0) {
					work = new Work(w, start, start + chunk - 1, step, taskid,
							i, nThreads, c);
					q.put(work);
					start += chunk;
					i = (i + 1) % nThreads;
			...
			case GUIDED: {
				...
				do {
					int m = Math.max(j / nThreads, chunk);
					if (m == 0) {
						work = new Work(w, start, end, step, taskid, i,
								nThreads, c);
						q.put(work);
						break;
					}
					...
					if (j < 0)
						m = chunk + j;
					work = new Work(w, start, start + m - 1, step, taskid, i,
							nThreads, c);
					q.put(work);
					start += m;
					i = (i + 1) % nThreads;
