...
public class TestOMPComputation {

	public static void main(String[] args) throws IOException {
		final long N = 10L; // 1000000L;
		...
		lc_omp.work(new _SumWork(), 1, THREADS);
		long end = System.currentTimeMillis();

		/* Results */
		boolean pass = (_SumWork.sum == expected);
...
class _SumWork implements IWork {
	...
	public boolean evaluate(Work w) {
		...
		for (;;) {
			try {
				...
				for(int i=0, j=0; i<100; i++) j *= i * j * Integer.parseInt(line);
			} catch (Exception ex) {
			}
			if (line == null)
				break;
